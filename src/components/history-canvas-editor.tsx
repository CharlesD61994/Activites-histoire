...
function minimumBlockSize(block: HistoryCanvasBlock, question: HistoryQuestion, width: number) {
  const scalableSize = scalableBlockSize(block, question);
  if (scalableSize) {
    const scale = Math.max(0.25, width / scalableSize.width);
    return {
      width: Math.max(100, scalableSize.width * 0.25),
      height: Math.max(48, scalableSize.height * scale)
    };
  }
...
export function HistoryCanvasEditor({ canvas, documents, question, onChange, onQuestionChange, availableActions, onActionChange, onAddDocument, onUpdateDocument, onDeleteDocument, contextPanel }: Props) {
  ...
  function onPointerMove(event: React.PointerEvent) {
    ...
    let x = drag.block.x;
    let y = drag.block.y;
    let width = drag.block.width;
    let height = drag.block.height;
    const horizontalMinimum = minimumBlockSize(drag.block, question, width).width;

    if (handle.includes("e")) width = clamp(drag.block.width + dx, horizontalMinimum, canvas.width - drag.block.x);
    if (handle.includes("w")) {
      x = clamp(drag.block.x + dx, 0, drag.block.x + drag.block.width - horizontalMinimum);
      width = drag.block.x + drag.block.width - x;
    }

    const verticalMinimum = minimumBlockSize(drag.block, question, width).height;
    if (handle.includes("s")) height = clamp(drag.block.height + dy, verticalMinimum, canvas.height - drag.block.y);
    if (handle.includes("n")) {
      y = clamp(drag.block.y + dy, 0, drag.block.y + drag.block.height - verticalMinimum);
      height = drag.block.y + drag.block.height - y;
    }

    updateBlock(drag.id, { x, y, width, height });
  }
  ...
  function renderBlock(block: HistoryCanvasBlock) {
    ...
    if (block.type === "interaction") return <HistoryInteractionEditor question={question} documents={documents} blockId={block.id} updateChoice={updateChoice} activateTextEditing={activateTextEditing} />;
    if (block.type === "validation") return <Button type="button" style={historyTextStyleToCss(block.textStyle)}><span contentEditable suppressContentEditableWarning onPointerDown={(event) => activateTextEditing(event, { kind: "block", id: block.id })} onFocus={(event) => activateTextEditing(event, { kind: "block", id: block.id })} onInput={(event) => updateBlock(block.id, { text: event.currentTarget.textContent ?? "" })}>{block.text ?? "Valider"}</span></Button>;
    if (block.type === "feedback") return <span className="history-canvas-reader-muted" style={historyTextStyleToCss(block.textStyle)} contentEditable suppressContentEditableWarning onPointerDown={(event) => activateTextEditing(event, { kind: "block", id: block.id })} onFocus={(event) => activateTextEditing(event, { kind: "block", id: block.id })} onInput={(event) => updateBlock(block.id, { text: event.currentTarget.textContent ?? "" })}>{block.text ?? "Feedback"}</span>;
    return <p style={historyTextStyleToCss(block.textStyle)} contentEditable suppressContentEditableWarning onPointerDown={(event) => activateTextEditing(event, { kind: "block", id: block.id })} onFocus={(event) => activateTextEditing(event, { kind: "block", id: block.id })} onInput={(event) => updateBlock(block.id, { text: event.currentTarget.textContent ?? "" })}>{block.text || "Texte"}</p>;
  }
...
function HistoryInteractionEditor({
  ...
  if (question.action === "short_text") {
    return (
      <div className="history-short-text-reader">
        <input readOnly value="" placeholder="Écrire un mot ou une courte phrase" onPointerDown={(event) => event.stopPropagation()} />
      </div>
    );
  }
  ...
  return (
    <div className="history-cloze-reader">
      ...
      {(question.clozeBlanks ?? []).map((blank) => <label key={blank.id}>Blanc {blank.label}<select value="" onPointerDown={(event) => event.stopPropagation()} onChange={() => undefined}><option value="">Choisir</option>{blank.options.map((option) => <option key={option.id} value={option.id}>{option.text}</option>)}</select></label>)}
    </div>
  );
}
