...
export function WorksheetReader({ sentence, persistenceKey, finishControl, onCompleteChange }: Props) {
  ...
  return <div className="worksheet-reader tree-reader">
    ...
    {page && <div className="tree-reader-page-viewport"><div className="tree-reader-page portrait document-template worksheet-reader-page" style={{ aspectRatio: "8.5 / 11", zoom }}>
      ...
      {(page.mainTitle?.enabled ?? true) && <div className="tree-analysis-document-title-banner" style={{ left: `${page.margins.left / W * 100}%`, right: `${page.margins.right / W * 100}%`, top: `${66 / H * 100}%` }}><div className="tree-analysis-document-title-line"><strong>{page.mainTitle?.prefix}</strong> <span>–</span> <em>{page.mainTitle?.title}</em></div>{page.mainTitle?.scoreTotal !== undefined && <div className="worksheet-main-title-score"><span>Total&nbsp;:</span><span className="worksheet-main-title-score-box"><b>/{page.mainTitle.scoreTotal}</b></span></div>}</div>}
      {page.taskCallout?.enabled && <div className="worksheet-task-callout" style={{ left: `${page.margins.left / W * 100}%`, right: `${page.margins.right / W * 100}%`, top: `${160 / H * 100}%` }}><Image className="worksheet-task-label-image" src="/worksheet-assets/ta-tache.png" alt="TA TÂCHE" width={80} height={32} unoptimized/><span className="worksheet-task-arrow" aria-hidden/><p>{page.taskCallout.text}</p></div>}
      ...
      {images.filter((item) => item.pageId === pageId).map((item) => { const mode = item.layoutMode ?? (item.wrapText ? "wrap" : "front"); const objectFit = item.objectFit === "contain" ? "contain" : item.documentTableId ? "fill" : "contain"; return <div key={item.id} className={`worksheet-page-image reader layout-${mode}`} style={{ left: `${item.x / W * 100}%`, top: `${item.y / H * 100}%`, width: `${item.width / W * 100}%`, height: `${item.height / H * 100}%` }}><Image style={{objectFit}} src={item.src} alt={item.alt} fill sizes="50vw" unoptimized/></div>; })}
      {scoreBoxes.filter((item) => (item.pageId ?? pages[0]?.id) === pageId).map((item) => <div key={item.id} className="tree-analysis-score-box worksheet-score-box" style={{ left: `${item.x / W * 100}%`, top: `${item.y / H * 100}%`, width: `${(item.width ?? 120) / W * 100}%`, height: `${(item.height ?? 42) / H * 100}%`, fontSize: `${(item.fontSize ?? Math.max(12, Math.min((item.height ?? 42) * .46, (item.width ?? 120) / (String(item.total).length + 5)))) / W * 100}cqw` }}><span>/{item.total}</span></div>)}
