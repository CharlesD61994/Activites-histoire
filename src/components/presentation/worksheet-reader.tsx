...
export function WorksheetReader({ sentence, persistenceKey, finishControl, onCompleteChange }: Props) {
  ...
  return <div className="worksheet-reader tree-reader">
    ...
    {page && <div className="tree-reader-page-viewport"><div className="tree-reader-page portrait document-template worksheet-reader-page" style={{ aspectRatio: "8.5 / 11", zoom }}>
      ...
      {images.filter((item) => item.pageId === pageId).map((item) => { const mode = item.layoutMode ?? (item.wrapText ? "wrap" : "front"); return <div key={item.id} className={`worksheet-page-image reader layout-${mode}`} style={{ left: `${item.x / W * 100}%`, top: `${item.y / H * 100}%`, width: `${item.width / W * 100}%`, height: `${item.height / H * 100}%` }}><Image style={{objectFit:item.objectFit??(item.documentTableId?"cover":"contain")}} src={item.src} alt={item.alt} fill sizes="50vw" unoptimized/></div>; })}
