...
export function historyQuestionMaxPoints(question: HistoryQuestion) {
  if (question.action === "choice_multiple") return Math.max(1, (question.choices ?? []).filter((choice) => choice.isCorrect).length);
  if (question.action === "classification") return Math.max(1, question.classificationItems?.length ?? 0);
  if (question.action === "matching") return Math.max(1, question.matchingPrompts?.length ?? 0);
  if (question.action === "chronological_order" || question.action === "timeline") return Math.max(1, question.timelineEvents?.length ?? 0);
